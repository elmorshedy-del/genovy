...
export async function fetchMondoDataset(source) {
  const response = await fetch(source.accessUrl, {
    headers: { 'user-agent': HTTP_CONSTANTS.userAgent }
  });
  if (!response.ok) {
    throw new Error(`MONDO fetch failed: ${response.status}`);
  }
  const payload = await response.json();
  const graph = payload.graphs?.[0];
  if (!graph) {
    throw new Error('MONDO graph payload missing');
  }
  ...
  for (const node of graph.nodes || []) {
    ...
    if (!curie.startsWith('MONDO:')) continue;

    entities.push({
      entityType: 'disease',
      canonicalCurie: curie,
      canonicalLabel: node.lbl || curie,
      description: extractDefinition(node),
      primarySourceKey: source.sourceKey,
      metadata: {
        subsets: node.meta?.subsets || [],
        nodeType: node.type || ''
      }
    });
  ...
  for (const edge of graph.edges || []) {
    ...
    if (!subjectCurie.startsWith('MONDO:') || !objectCurie.startsWith('MONDO:')) continue;
    if (edge.pred !== 'is_a') continue;

    const sourceRecordKey = `edge:${subjectCurie}:${edge.pred}:${objectCurie}`;
    relationships.push({
      predicateKey: 'is_a',
      subjectRef: { curie: subjectCurie, entityType: 'disease', label: subjectCurie },
      objectRef: { curie: objectCurie, entityType: 'disease', label: objectCurie },
      qualifiers: {},
      evidence: {
        sourceRecordKey,
